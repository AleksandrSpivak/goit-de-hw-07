...
# 7. Додатковий крок - вивід з таблиці
def print_table_content():
    hook = MySqlHook(mysql_conn_id=connection_name)
    result = hook.get_records("SELECT * FROM olympic_medal_counts;")
    for row in result:
        print(row)

print_table = PythonOperator(
    task_id='print_table',
    python_callable=print_table_content,
    dag=dag,
)
...
check_recent_entry >> print_table
